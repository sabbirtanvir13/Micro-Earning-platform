...
const BuyerDashboard = () => {
  ...
  // Demo tasks data
  const demoTasks = [
    { _id: '1', title: 'Data Entry - Product Catalog', status: 'open', currentWorkers: 12, requiredWorkers: 20, coinsPerWorker: 50 },
    { _id: '2', title: 'Content Writing - Blog Posts', status: 'in_progress', currentWorkers: 8, requiredWorkers: 15, coinsPerWorker: 120 },
    { _id: '3', title: 'Image Tagging for AI Training', status: 'completed', currentWorkers: 30, requiredWorkers: 30, coinsPerWorker: 80 },
    { _id: '4', title: 'Social Media Content Creation', status: 'open', currentWorkers: 15, requiredWorkers: 25, coinsPerWorker: 100 },
    { _id: '5', title: 'Translation - English to Spanish', status: 'completed', currentWorkers: 10, requiredWorkers: 10, coinsPerWorker: 150 },
  ];

  useEffect(() => {
    if (isDemoMode) {
      // In demo mode, use demo data
      const active = demoTasks.filter((t) => t.status === 'open' || t.status === 'in_progress').length;
      const completed = demoTasks.filter((t) => t.status === 'completed').length;
      const totalWorkers = demoTasks.reduce((sum, task) => sum + task.currentWorkers, 0);
      const totalSpent = demoTasks.reduce((sum, task) => sum + (task.currentWorkers * task.coinsPerWorker), 0);

      setStats({
        totalSpent,
        activeTasks: active,
        completedTasks: completed,
        totalCoins: user?.coins || 5000,
        totalWorkers,
      });
      setLoading(false);
    } else {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-200 rounded-2xl p-6 hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="text-gray-700 text-sm font-medium mb-1">Available Balance</div>
          <div className="text-3xl font-black text-blue-800">{stats.totalCoins} <span className="text-lg">coins</span></div>
        </div>
        ...
        <div className="bg-gradient-to-br from-red-50 to-pink-50 border border-red-200 rounded-2xl p-6 hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="text-gray-700 text-sm font-medium mb-1">Total Invested</div>
          <div className="text-3xl font-black text-red-800">{stats.totalSpent} <span className="text-lg">coins</span></div>
        </div>
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-orange-50 border border-yellow-200 rounded-2xl p-6 hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="text-gray-700 text-sm font-medium mb-1">Active Tasks</div>
          <div className="text-3xl font-black text-yellow-800">{stats.activeTasks}</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6 hover:shadow-lg transition-shadow duration-300">
          ...
          <div className="text-gray-700 text-sm font-medium mb-1">Completed Tasks</div>
          <div className="text-3xl font-black text-green-800">{stats.completedTasks}</div>
        </div>
      </div>
